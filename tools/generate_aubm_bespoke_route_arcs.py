...
from aubm_menu_safety import MENU_IDS, ensure_menu_exits
    ...
    Route(
        ...
        focuses=(
            ...
            Focus(
                ...
                "A Great Power Negotiates Directly with Delhi", "A decisive great-power result or two Indian national theatres make India a balancer in fact. With LIBERATOR1 enabled, ending the Japanese war after the Southeast Asian operational victory and two actual sovereign partnerships is an alternative, liberation-based completion. No second primary charter is awarded.",
                "OR = { flag = ind_aubm_decisive_great_power flag = ind_aubm_britain_major_victory flag = ind_aubm_germany_major_victory flag = ind_aubm_soviet_major_victory flag = ind_aubm_japan_major_victory flag = ind_aubm_america_major_victory flag = ind_lib1_southern_peace }",
...
def render_status(route: Route) -> str:
    ...
    description = route.status_desc
    if route.key == "japan":
        description = (
            "Cancel closes this board without changing your compact. The multi-theatre ledger opens after India enters war and chooses the Delhi-Tokyo Wartime Charter, from 1937 onward; waiting at peace will not unlock it. "
            "Equal Asian Command covers the Philippines and China; Indian Ocean First covers Suez and Africa; Tripartite coordination responds to German setbacks. Existing southern and northern partnership campaigns remain in force."
        )
    lines.extend([
...
def render() -> str:
    ...
    return ensure_menu_exits(content, MENU_IDS[OUTPUT.name])
