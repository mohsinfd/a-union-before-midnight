...
ROUTES = (
    Route(
        ...
        (
            ...
            Focus("anticolonial", "Anti-Colonial Liberation Mandate", "India Turns Allied War into Decolonization", "A western or African result lets Delhi tie military cooperation to sovereign postwar governments. This strengthens Indian settlement standing without granting territory automatically.", "OR = { flag = ind_aubm_national_western_victory flag = ind_aubm_regional_victory_eth flag = ind_aubm_regional_victory_saf flag = ind_aubm_regional_victory_per flag = ind_aubm_regional_victory_irq flag = ind_aubm_regional_victory_sau }", ("belligerence value = -3", "dissent value = -2", "setflag which = ind_aubm_coalition_consultation")),
    ...
    Route(
        ...
        (
            ...
            Focus("imperial", "Dismantle Britain's imperial system", "India Breaks the Imperial System from the East", "A British, southern or western victory gives Delhi the decisive claim in the Indian Ocean while Germany remains a continental partner.", "OR = { flag = ind_aubm_britain_limited_victory flag = ind_aubm_britain_major_victory flag = ind_aubm_major_annex_victory_eng flag = ind_aubm_national_southern_victory flag = ind_aubm_national_western_victory }", ("money value = 300", "dissent value = -2", "setflag which = ind_aubm_coalition_consultation")),
            Focus("southern", "Win the southern resource race", "Delhi Wins the Southern Race", "Indian command has secured the southern resource arc before another coalition power could define it. The result becomes sovereign settlement credit.", "OR = { flag = ind_aubm_national_southern_victory flag = ind_aubm_regional_victory_sia flag = ind_aubm_regional_victory_u05 flag = ind_aubm_regional_victory_hol flag = ind_aubm_regional_victory_ast }", ("supplies value = 800", "dissent value = -1", "setflag which = ind_aubm_victory_sovereign_credit")),
    ...
    Route(
        ...
        (
            ...
            Focus("antiimperial", "Anti-Imperial Ocean War", "India Converts Socialist War into Oceanic Liberation", "A British, southern or western result gives Delhi anti-colonial leverage distinct from Soviet territorial interests.", "OR = { flag = ind_aubm_britain_limited_victory flag = ind_aubm_britain_major_victory flag = ind_aubm_major_annex_victory_eng flag = ind_aubm_national_southern_victory flag = ind_aubm_national_western_victory }", ("belligerence value = -3", "dissent value = -2", "setflag which = ind_aubm_coalition_consultation")),
            Focus("republican", "A Republican Asian Order", "Delhi Establishes an Asian Republican Claim", "An Indian result in China, Siam or against Japan establishes a specifically Asian socialist-republican programme outside Moscow's direct command.", "OR = { flag = ind_aubm_japan_limited_victory flag = ind_aubm_japan_major_victory flag = ind_aubm_major_annex_victory_jap flag = ind_aubm_pacific_capital_victory flag = ind_aubm_regional_victory_chi flag = ind_aubm_regional_victory_chc flag = ind_aubm_regional_victory_sia }", ("money value = 250", "dissent value = -1", "setflag which = ind_aubm_coalition_consultation")),
    ...
    Route(
        ...
        (
            Focus("southern", "Indian Southern Sphere", "Tokyo Recognizes the Indian Southern Sphere", "India has won the published southern theatre. The result confirms Indian primacy from Burma through Malaya and the East Indies; country-specific settlements still decide sovereignty.", "OR = { flag = ind_aubm_jp_southern_victory flag = ind_aubm_national_southern_victory }", ("supplies value = 800", "dissent value = -2", "setflag which = ind_aubm_jp_india_full_sphere")),
